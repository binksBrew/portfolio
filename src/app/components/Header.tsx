...
import { usePathname } from 'next/navigation'
import Link from 'next/link'
import { cn } from '../utils/cn' // utility to combine classes
...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 px-12 py-8 flex justify-between items-center text-white">
      ...
      <Link
        href="/home"
        className="-mt-5 font-inter font-black text-[18px] uppercase tracking-wide"
      >
        ISHA <span className="font-normal italic">Gaaru</span>
      </Link>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'w-[100px] h-[29px] px-[2px] py-[4px] flex items-center gap-2 transition-all duration-150 hover:text-white',
                isActive ? 'bg-[#E4F2F5] text-black' : 'text-[#162b22]'
              )}
            >
              {isActive && (
